...
## 🌊 US-024 – Standortgestützte Pegelinformation im Dashboard
...
Als Nutzer eines maritimen Wetterdashboards  
möchte ich den nächstgelegenen Pegelort sowie die nächsten Hoch- und Niedrigwasserereignisse direkt im Dashboard sehen  
damit ich die aktuelle Wasserstandssituation schnell und ohne externe Suche einordnen kann

---

### 🎯 Zielbild

- Die App nutzt die verfügbare GPS-Position des Geräts, sofern sie verfügbar ist
- Der nächstgelegene Pegelort wird automatisch ermittelt
- Die nächsten Hoch- und Niedrigwasserereignisse werden kompakt dargestellt
- Die Darstellung bleibt klein, attraktiv und in den bestehenden Dashboard-Workflow integriert
- Optional kann ein Barometer- oder Drucktrend-Hinweis als zusätzlicher Kontext angezeigt werden

---

### 🗂️ Datenquelle

- BSH Water Level Forecast API
- Endpunkt: https://gdi.bsh.de/ldproxy/rest/services/WaterLevelForecast/collections/waterlevelforecastdata/items
- Ziel ist die Nutzung der offiziellen Wasserstandsvorhersage für relevante Pegelstationen

---

### ✅ Akzeptanzkriterien

- [ ] Die App kann die GPS-Position des Geräts nutzen, sofern sie verfügbar ist
- [ ] Aus der verfügbaren Stationenliste wird der nächstgelegene Pegelort bestimmt
- [ ] Die nächsten Hoch- und Niedrigwasserereignisse werden kompakt dargestellt
- [ ] Die Darstellung erfolgt als kleines Overlay oder Widget im bestehenden Dashboard-Layout
- [ ] Die bestehende Navigation, Lightbox und Kartenlogik bleiben funktionsfähig
- [ ] Die Darstellung bleibt minimal, lesbar und nicht störend
- [ ] Ein optionaler Druck-/Barometer-Kontext kann ergänzend angezeigt werden, ohne die Hauptfunktion zu überladen

---

### 💡 Hinweise zur UX

- Die Anzeige sollte eher wie ein kompakter „Insight“-Block wirken als wie ein separates Menü
- Fokus liegt auf den nächsten relevanten Ereignissen und dem aktuellen Trend
- Die Funktion soll zunächst für eine sinnvolle Auswahl von Stationen und nicht als große, komplexe Datenansicht umgesetzt werden
