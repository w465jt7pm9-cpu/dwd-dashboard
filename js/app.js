document.addEventListener('DOMContentLoaded', () => {
  ...
  function buildMoonPhaseEventsForYear (year) {
    if (!Number.isFinite(year)) {
      return { newMoonDates: [], fullMoonDates: [], firstQuarterDates: [], lastQuarterDates: [] }
    }
    ...
    while (true) {
      ...
      const candidates = [
        ...
        [events.firstQuarterDates, newMoonMs + (GEOZEIT_MOON_CYCLE_DAYS / 4) * GEOZEIT_DAY_MS],
        [events.fullMoonDates, newMoonMs + (GEOZEIT_MOON_CYCLE_DAYS / 2) * GEOZEIT_DAY_MS],
        [events.lastQuarterDates, newMoonMs + (GEOZEIT_MOON_CYCLE_DAYS * 3 / 4) * GEOZEIT_DAY_MS]
      ]

      candidates.forEach(([targetArray, eventMs]) => {
        if (eventMs >= yearStartMs - marginMs && eventMs <= yearEndMs + marginMs) {
  ...
  function buildNordseeTideIndicatorHeaderRowMarkup (slots) {
    ...
    const slotYears = slotDates
      .filter(slotDate => slotDate instanceof Date && !Number.isNaN(slotDate.getTime()))
